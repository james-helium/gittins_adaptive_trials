#this script combines the previously written ER and GI simulations of 3-armed trials
#(see script 7 for the 2-armed case)
#and simulate the two designs in different trial conditions 
#we will produce visualisations in a different script
...
#since this is 3-armed, we vary both in getting the data
#during visualisation we can decide how we format them

#a function that outputs a data frame of x1 = mu_1, x2 = mu_2, and y = power, accuracy, prop superior; for ER trials
simulate_mu_range_ER <- function(N, mu_0, mu_1_bounds, mu_2_bounds, mu_step, R){
  
  #get the mu sequences into lists
  mu_1_list <- seq(mu_1_bounds[1], mu_1_bounds[2], by = mu_step)
  mu_2_list <- seq(mu_2_bounds[1], mu_2_bounds[2], by = mu_step)
  #create empty frame to store outcomes
  output <- data.frame(mu_1 = rep(mu_1_list, each = length(mu_2_list)), 
                       mu_2 = rep(mu_2_list, time = length(mu_1_list)),
                       Alpha = NA, Power = NA, 
                       H1.Estimate.1.SD = NA, H1.Estimate.2.SD = NA, 
                       H1.Prop.Superior = NA, H1.ETO.Change.Percent = NA)
  #simulate the null
  null.true <- simulate_trials_ER(N, mu_0, mu_0, mu_0, R,
                                  previous_message = 'Simulating ER under NULL |')
  
  #go through each row to simulate the alte
  for(row in 1:nrow(output)){
    
    mu_1 <- output$mu_1[row]
    mu_2 <- output$mu_2[row]
    cutoff <- 0.05/2 #bonferroni correction
    
    #simulate by ER
    alte.true <- simulate_trials_ER(N, mu_0, mu_1, mu_2, R,
                                    previous_message = 
                                      paste('Simulating ER under ALTE: mu_1 =',mu_1,
                                            '| mu_2 =',mu_2,'| row',row,'out of',nrow(output),'|'))
    ...
    #accuracy
    output$H1.Estimate.1.SD[row] <- sd(alte.true$Estimate.1)
    output$H1.Estimate.2.SD[row] <- sd(alte.true$Estimate.2)
    #patient outcomes
    output$H1.Prop.Superior[row] <- mean(alte.true$Prop.Superior)
    output$H1.ETO.Change.Percent[row] <- 
      mean((alte.true$Total.Outcome - (mu_0*N/3+mu_1*N/3+mu_2*N/3))/(mu_0*N/3+mu_1*N/3+mu_2*N/3)*100)
  }
...
simulate_mu_range_GI_constraint <- function(N, mu_0, mu_1_bounds, mu_2_bounds, mu_step, R, implicit_prior_n = 2, k = 5){
  ...
  output <- data.frame(mu_1 = rep(mu_1_list, each = length(mu_2_list)), 
                       ...
                       H1.Prop.Superior = NA, H1.ETO.Change.Percent = NA)
  #simulate the null
  null.true <- simulate_trials_GI_constraint(N, mu_0, mu_0, mu_0, R, implicit_prior_n, k,
                                             previous_message = paste('Simulating GI k =',k,'under NULL |'))
  ...
  for(row in 1:nrow(output)){
    ...
    #simulate by GI
    alte.true <- simulate_trials_GI_constraint(N, mu_0, mu_1, mu_2, R, implicit_prior_n, k,
                                               previous_message = 
                                                 paste('Simulating GI k =',k,'under ALTE: mu_1 =',mu_1,
                                                       '| mu_2 =',mu_2,'| row',row,'out of',nrow(output),'|'))
...
#set arbiturary parametres
N = 100
mu_0 = 0.4
mu_1_bounds = c(0.1,0.9)
mu_2_bounds = c(0.2,1.0)
mu_step = 0.02
R = 1000
#get trial property outcomes
mu_range_outcome_ER <- simulate_mu_range_ER(N, mu_0, mu_1_bounds, mu_2_bounds, mu_step, R)
mu_range_outcome_GI_const_5 <- simulate_mu_range_GI_constraint(N, mu_0, mu_1_bounds, mu_2_bounds, 
                                                               mu_step, R, implicit_prior_n = 2, k = 5)
mu_range_outcome_GI_const_9 <- simulate_mu_range_GI_constraint(N, mu_0, mu_1_bounds, mu_2_bounds, 
                                                               mu_step, R, implicit_prior_n = 2, k = 99)
#for near-optimal case we only constrain for k = N / 2
mu_range_outcome_GI_near_op <- simulate_mu_range_GI_constraint(N, mu_0, mu_1_bounds, mu_2_bounds, 
                                                               mu_step, R, implicit_prior_n = 2, k = N/2)
